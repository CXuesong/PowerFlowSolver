...
namespace PowerSolutions {
	namespace ObjectModel {
		...
		private:
			...
			void* m_Tag;
		private:	//internal
			bool NotifyObjectIndexChanged(NetworkObject* obj, int oldIndex, int newIndex);
		public:
			const std::unordered_map<int, std::shared_ptr<Bus>>& Buses()	//总线列表。
			{ return m_Buses; }
			const std::unordered_set<std::shared_ptr<Component>>& Components()		//网络元件的列表。
			{ return m_Components; }
			void* Tag() const { return m_Tag; }		//应用程序自定义的附加信息。
			void Tag(void* val) { m_Tag = val; }
		public:
			void Attach(std::shared_ptr<NetworkObject> obj);
			void Detach(std::shared_ptr<NetworkObject> obj);
