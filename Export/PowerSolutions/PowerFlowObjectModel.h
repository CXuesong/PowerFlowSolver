...
namespace PowerSolutions {
	namespace ObjectModel
	{
		...
		//含有非标准变比的无铁损三绕组变压器。
		class ThreeWindingTransformer final : public TriPortComponent, public IContainer
		{
		private:
		...
		public:
			//获取此变压器使用的一次侧内部公共母线。
			std::shared_ptr<Bus> CommonBus() const { return m_CommonBus; }
			//获取此变压器使用的一次侧内部变压器。
			std::shared_ptr<Transformer> Transformer1() const { return m_Transformer1; }
			//获取此变压器使用的二次侧内部变压器。
			std::shared_ptr<Transformer> Transformer2() const { return m_Transformer2; }
			//获取此变压器使用的三次侧内部变压器。
			std::shared_ptr<Transformer> Transformer3() const { return m_Transformer3; }
		...
		private:	//基础结构。
			...
			virtual std::shared_ptr<NetworkObject> ChildAt(int index) const override;
		protected:
			virtual NetworkObject* CloneInstance() const override;
